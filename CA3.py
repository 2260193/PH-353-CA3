...
def compute_action(path, m, mu, time_step):
    action = 0.0
    
    for i in range(1, len(path)):
        kinetic = 0.5 * m * ((path[i] - path[i-1]) / time_step)**2
        potential = V(path[i], mu)
        action += (kinetic - potential) * time_step
    
    return action

for mu,m in values:
    def metropolis_path(N, proposal_step, num_samples, mu, m, time_step):
        paths = []
        ...
        for _ in range(num_samples):
            ...
            current_action = compute_action(path, m, mu, time_step)
            
            for step in range(N):
                new_path = path.copy()
                new_path[step] += np.random.uniform(-proposal_step, proposal_step)
                
                new_action = compute_action(new_path, m, mu, time_step)
    ...
    paths = metropolis_path(N, proposal_step, num_samples, mu, m, time_step)
...
def V(x, mu, lambda_):
    return 0.5 * mu * x**2 + lambda_ * (x**4)


def compute_action_an(path, m, mu, lambda_, time_step):
    action = 0.0
...
for mu,m,lambda_ in values:
    def metropolis_path_an(N, proposal_step, num_samples, mu, m, lambda_, time_step):
        paths = []
        ...
        for _ in range(num_samples):
            ...
            current_action = compute_action_an(path, m, mu, lambda_, time_step)
            
            for step in range(N):
                new_path = path.copy()
                new_path[step] += np.random.uniform(-proposal_step, proposal_step)
                
                new_action = compute_action_an(path, m, mu, lambda_, time_step)
                delta_action = new_action - current_action
    ...
    paths = metropolis_path_an(N, proposal_step, num_samples, mu, m, lambda_, time_step)
